...
int main(int argc, char *argv[]) {
  ...
  if (rank == 0) {
    ...
    // Broadcast each row of matrix_b to all processes
    for (int i = 0; i < MATRIX_SIZE; i++) {
      ierr = MPI_Bcast(matrix_b[i], MATRIX_SIZE, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    }

    // Root process sends rows of matrix_a to other processes
    int row_a_counter = 0;
    ...
    for (int i = 0; i < MATRIX_SIZE; i++) {
      ...
      //free process takes next row of matrix_a
      if (row_a_counter < MATRIX_SIZE) {
        ierr = MPI_Send(matrix_a[row_a_counter], MATRIX_SIZE, MPI_DOUBLE, sender, row_a_counter + 1, MPI_COMM_WORLD);
        row_a_counter++;
      } else {
  ...
  else 
{
    ...
    for (int i = 0; i < MATRIX_SIZE; i++) {
      // sending and receiving with bcast
      ierr = MPI_Bcast(matrix_b[i], MATRIX_SIZE, MPI_DOUBLE, 0, MPI_COMM_WORLD);
    }
